...
CLocalBlast::Run()
{
    ...
    BlastScoreBlk* sbp = m_InternalData->m_ScoreBlk->GetPointer();
    ScoreBlk = sbp;

    //ceb Save this object for use in consolidating i/o
    // Dump the QueryInfo data structure.
    BlastQueryInfo* bqi = m_InternalData->m_QueryInfo;
    //BlastQueryInfo* bqi = BlastQueryInfoDup(m_InternalData->m_QueryInfo);
    QueryInfo = bqi;
