...
int main(int argcs, char* pArgs[])
{
  ...
  while ( line_ptr != NULL )
      ...
      else
	{
	  ...
	  while ( !got_newline ) 
	    {
	      // Check memory, realloc if necessary.
	      if ( seq_len[current_seq] + seq_length >= seq_alloc[current_seq] )
		{
		  // Add on the currently read line length to the sequence.
		  db_seq[current_seq] = (char*) realloc( (void*) db_seq[current_seq], (seq_alloc[current_seq] + seq_length ) );
		  if ( db_seq[current_seq] == NULL )
		    {  printf("Error: Could not reallocate db_seq[%llu].\n",current_seq);  exit(1);  }

		  seq_alloc[current_seq] += seq_length;
		}

	      // Save the line to the sequence line
	      strncpy( &(db_seq[current_seq][seq_len[current_seq]]), line, seq_length );
	      
	      seq_len[current_seq] += seq_length;
